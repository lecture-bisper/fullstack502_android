package btic.fullstack502.app_20250729
...
import btic.fullstack502.app_20250729.databinding.ActivityRes1Binding

class Res1Activity : AppCompatActivity() {
...
//    Build.VERSION.SDK_INT : 현재 사용하는 안드로이드 OS의 버전 확인
//    Build.VERSION_CODES : 안드로이드 OS 종류, Build.VERSION_CODES.R 은 API 30(안드로이드 11) 버전을 의미함
//    현재 안드로이드 기기의 버전이 지정한 안드로이드 API 버전보다 같거나 높을 경우 신규 방식을 사용
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
      val windowMetrics: WindowMetrics = windowManager.currentWindowMetrics
      binding.tv09.text = "width : ${windowMetrics.bounds.width()}, height : ${windowMetrics.bounds.height()}"
    }
//    현재 안드로이드 기기의 버전이 지정한 안드로이드 API 버전보다 낮을 경우 예전 방식을 사용
    else {
...
// 문제 1) 책 260 ~ 269 페이지의 메신저 앱 인트로 화면 만들기를 진행하세요
